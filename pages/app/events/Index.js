...
export default function EventsIndex() {
  ...
  const onCreate = async (e) => {
    ...
    try {
      ...
      const payload = { name: form.name.trim(), owner_email: ownerEmail, event_date: form.event_date || null, venue: form.venue || null };
      const { data: created, error: insErr } = await supabase.from('events').insert(payload).select('id').single();
      if (insErr) throw insErr;
      if (created?.id) {
        await supabase.from('event_members').insert({ event_id: created.id, member_email: ownerEmail, role: 'owner' });
      }
      ...
      <div style={{border:'1px solid #ddd',borderRadius:12,padding:12}}>
        ...
        <form onSubmit={onCreate} style={{display:'grid',gap:10}}>
          <div>
            ...
            <input placeholder="Ex.: Casamento Marina & Caio" value={form.name} onChange={e=>setForm({ ...form, name: e.target.value })} required style={{width:'100%',padding:'8px',border:'1px solid #ccc',borderRadius:8}} />
          </div>
          <div>
            <div style={{fontSize:12,marginBottom:4}}>Data (opcional)</div>
            <input type="date" value={form.event_date} onChange={e=>setForm({ ...form, event_date: e.target.value })} style={{width:'100%',padding:'8px',border:'1px solid #ccc',borderRadius:8}} />
          </div>
          <div>
            <div style={{fontSize:12,marginBottom:4}}>Local (opcional)</div>
            <input placeholder="Salão, igreja, cidade…" value={form.venue} onChange={e=>setForm({ ...form, venue: e.target.value })} style={{width:'100%',padding:'8px',border:'1px solid #ccc',borderRadius:8}} />
          </div>
          <div><button disabled={sending} style={{padding:'10px 14px',border:'1px solid #ccc',borderRadius:8,background: sending ? '#f3f4f6' : '#fff'}}>{sending ? 'Criando…' : 'Criar evento'}</button></div>
        </form>
          ...
          <tbody>
            ...
            {!loading && items.map(ev => (
              <tr key={ev.id} style={{borderBottom:'1px solid #f1f1f1'}}>
                ...
                <td style={{padding:'8px'}}><Link href={`/app/events/${ev.id}`}><span style={{padding:'6px 10px',border:'1px solid #ccc',borderRadius:8}}>Abrir módulos</span></Link></td>
              </tr>
            ))}
          </tbody>
        </table>
