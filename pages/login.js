...
// validação simples p/ cadastro com senha forte (já que "leaked passwords" é Pro)
function validateStrongPassword(pwd) {
  ...
  if (!/[^A-Za-z0-9]/.test(pwd)) return 'Inclua ao menos 1 símbolo (ex.: !@#&*).';
  return null;
}
...
export default function Login() {
  ...
  // Se já estiver logado, manda direto pra /app/events
  useEffect(() => {
  ...
  // LOGIN COM SENHA → vai para /app/events
  const onLoginPassword = async (e) => {
    ...
    router.replace('/app/events'); // <- importante
  };

  // CRIAR CONTA (com validação de força) → depois entrar em "Com senha"
  ...
  // UI simples (inline styles p/ funcionar até sem Tailwind)
  return (
